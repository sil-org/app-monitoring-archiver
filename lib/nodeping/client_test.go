...
func TestNew(t *testing.T) {
	...
	client, err := New(ClientConfig{Token: "abc123"})
	if err != nil {
		t.Error(err)
		t.FailNow()
	}
...
func TestListChecks(t *testing.T) {
	t.Skip("not suitable for automated testing")

	client, err := New(ClientConfig{Token: os.Getenv("NODEPING_TOKEN")})
	if err != nil {
		t.Error(err)
		return
	}
...
func TestListChecksMock(t *testing.T) {
	client, err := New(ClientConfig{Token: "mock"})
	if err != nil {
		t.Error(err)
		return
	}
...
func TestGetContactGroupIDFromName(t *testing.T) {
	npClient, _ := New(ClientConfig{Token: "mock"})
...
`
	...
	resultsID, err := npClient.GetContactGroupIDFromName("CGList2")
	if err != nil {
		t.Error(err.Error())
		return
	}
...
`
	...
	checkLabels, checkIDs, _ := npClient.GetCheckIDsAndLabels(cgID)
